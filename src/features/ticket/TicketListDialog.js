...
import {
    ...
    Box, Typography, IconButton, Table, TableHead, TableBody, TableRow, TableCell, Select, MenuItem, CircularProgress, Chip
} from '@mui/material';
...
export default function TicketListDialog({ open, unit, onClose, onTicketsChanged }) {
    ...
    const [openTicketForm, setOpenTicketForm] = useState({ open: false, ticket: null });
        ...
        (async () => {
            ...
            if (ids.length > 0) {
                const { data: files } = await supabase
                    .from('attachments')
                    .select('id, ticket_id, file_url, file_type, uploaded_at, is_fix_act, storage_path')
                    .in('ticket_id', ids);
    ...
    const handleStatusChange = async (ticketId, newStatusId) => {
        await supabase
            ...
            .eq('id', ticketId);
        setTickets(ts => ts.map(tk => tk.id === ticketId ? { ...tk, status_id: newStatusId } : tk));
    ...
    const handleUploadFile = async (ticketId, e) => {
        ...
        try {
            const { path, url, type } = await uploadAttachment(file, projectId, ticketId);
    ...
    return (
        <>
            <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth PaperProps={{ sx: { minWidth: 700 } }}>
                                ...
                                <TableBody>
                                    {tickets.map((t, i) => (
                                        <TableRow key={t.id}>
                                            ...
                                            <TableCell sx={{ minWidth: 160, maxWidth: 300 }}>{t.title}</TableCell>
                                            ...
                                            <TableCell sx={{ minWidth: 180 }}>
                                                <Box sx={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
                                                    {(attachments[t.id]?.filter(a => a.is_fix_act) || []).map((a, idx) => (
                                                        <Chip
                                                            key={a.file_url}
                                            ...
                                            <TableCell>
                                                <Button
                                                    ...
                                                    onClick={() => setOpenTicketForm({ open: true, ticket: t })}
                                                >
                                                    Подробно
                                                </Button>
                                            </TableCell>
